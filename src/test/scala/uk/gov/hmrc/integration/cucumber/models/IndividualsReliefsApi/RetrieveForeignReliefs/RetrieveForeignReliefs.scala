/*
 * Copyright 2022 HM Revenue & Customs
 *
 */

package uk.gov.hmrc.integration.cucumber.models.IndividualsReliefsApi.RetrieveForeignReliefs

import play.api.libs.json.{Json, OFormat}
import uk.gov.hmrc.integration.cucumber.models.hateoas.HateoasLink

case class RetrieveForeignReliefs(submittedOn: String,
                                  foreignTaxCreditRelief: Option[ForeignTaxCreditRelief],
                                  foreignIncomeTaxCreditRelief: Option[Seq[ForeignIncomeTaxCreditRelief]],
                                  foreignTaxForFtcrNotClaimed: Option[ForeignTaxForFtcrNotClaimed],
                                  links: Seq[HateoasLink])

object RetrieveForeignReliefs {
  implicit val format: OFormat[RetrieveForeignReliefs] = Json.format[RetrieveForeignReliefs]
}
