@Auth @IndividualsChargesApiAuth @IndividualsChargesApi @All

Feature: Test auth for retrieve individuals-charges-api on a single endpoint
  ...
  Scenario: Successful request - individual
    ...
    And a request is made to PUT to url: PensionChargesUrl, with the JSON body: AmendIndividualsChargesPensionsMaximum, and the gov-test-scenario: None
    When a request is made to GET to url: PensionChargesUrl, and the gov-test-scenario: None
    Then the response code is 200
    And a request is made to DELETE to url: PensionChargesUrl, and the gov-test-scenario: None
