@Auth @IndividualsStateBenefitsApiAuth @IndividualsStateBenefitsApi @All

Feature: Test auth for individuals-state-benefits-api on a single endpoint
  ...
  Scenario: Agent not authorised by client
    ...
    When a request is made to PUT to url: IndividualsStateBenefitsPathIdUrl, with the JSON body: iStateBenefitsAmendRequestT, and the gov-test-scenario: None
    Then the response code is 403
    And the response body is CLIENT_OR_AGENT_NOT_AUTHORISED
